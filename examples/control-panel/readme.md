# Control Panel
In this example, we set up a simple UI to control the printer from. With the customizability of user interfaces in p5, you can add custom components to test or experiment with repeated actions. In this example you can manually move the print head to an absolute position or use the buttons to move relatively by the `step size`. Be sure to home the printhead before trying to send an absolute position!
...
function setupUI() {
  /************** CONTROLS UI *************
  * this UI isn't made very well, it's unresponsive!
