...
class Fab {
  constructor(config = defaultPrinterSettings) {
    ...
    // Imported G-code visualization
    this.importedGCode = false;
    this.importedGCodeCommands = [];
  }
  ...
  render() {
    ...
    if (this.importedGCode) {
      let importedToolpathPos = new p5.Vector(0, 0, 0);
      beginShape(LINES);
      for (let command of this.importedGCodeCommands) {
        let coords = this.parseCommand(command);
        if (coords.x !== null && coords.y !== null && coords.z !== null) {
          vertex(importedToolpathPos.x, importedToolpathPos.y, importedToolpathPos.z);
          vertex(coords.x, coords.y, coords.z);
          importedToolpathPos = importedToolpathPos.set([coords.x, coords.y, coords.z]);
        }
      }
      endShape();
    }
  ...
  saveGCode() {
    ...
    a.href = url;
    a.download = "fab.gcode";
    a.click();
  }

  loadGCode(gcodeContent) {
    this.importedGCode = true;
    this.importedGCodeCommands = gcodeContent.split("\n");
  }  

  parseCommand(command) {
    let coords = { x: null, y: null, z: null };
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (part.startsWith("X")) {
        ...
        coords.z = parseFloat(part.substring(1)); // Swap Y and Z
      } else if (part.startsWith("Z")) {
        coords.y = -parseFloat(part.substring(1)); // Invert Z
      }
    }
    return coords;
  }

  flavorParser(command) {
    const setupPattern = [
      ...
      'G1 F1500 ; Set the movement speed'
    ];
  
    const commandsToSkip = new Set(['M83', 'M107', 'G28', 'G92', 'M104', 'M140', 'M109', 'M190', 'G1']);
    const processedCommands = commandsArray.filter(command => {
      const commandCode = command.split(' ')[0];
      return !commandsToSkip.has(commandCode);
    });
  
    return [...setupPattern, ...processedCommands];
  }
  ...
  showCoordinates(x, y, z) {
    let xCoord = x.toFixed(2);
    let yCoord = y.toFixed(2);
    let zCoord = z.toFixed(2);
    let coords = `X: ${xCoord}, Y: ${yCoord}, Z: ${zCoord}`;
    return coords;
  }
}

function windowResized() {
  _Pos = _fab.cameraPosition;
