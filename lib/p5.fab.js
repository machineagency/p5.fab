 ...
 * should load in from config file
 */
class Fab {
    constructor({ baudRate = 115200, nozzleRadius = 0.4 / 2, filamentRadius = 1.75 / 2, maxX = 220, maxY = 220, maxZ = 250, autoConnect = true } = {}) {
        ...
        this.cameraOrientation = new p5.Vector(this.camera.centerX, this.camera.centerY, this.camera.centerZ);
    }
        ...
        _fab.commands.forEach((cmd) => {
            ...
            cmd.forEach((c) => {
                ...
                switch (c.charAt(0)) {
                    ...
                    case ";":
                        if (val == 'prime' || val == 'present') {  // || val == 'intro' to remove intro line
                            newV = null;
                            return;
                        }
                }
    ...
    render() {
        ...
        if (!this.model) {
            ...
            for (let v in this.vertices) {
                ...
                if (vertexData.command == "G0") {
                    toolpathPos = toolpathPos.set([vertexData.vertex.x, vertexData.vertex.y, vertexData.vertex.z]);
        ...
        if (_recoverCameraPosition) {
            this.camera.setPosition(this.cameraPosition.x, this.cameraPosition.y, this.cameraPosition.z);
            _recoverCameraPosition = false;
            this.camera.lookAt(this.cameraOrientation.x, this.cameraOrientation.y, this.cameraOrientation.z);
        }
