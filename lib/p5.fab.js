...
class Fab {
  ...
  interactivePrintStream() {
    if (this.commandStream.length > 0) {
      this.isPrinting = true;
      let commandToSend = this.updateWithMidiValues(this.commandStream[0]);
  ...
  updateWithMidiValues(cmd) {
    ...
    if (this.fabscribe) {
      this.midiRecording.extrusionMultiplier = this.midiRecording.extrusionMultiplier || [];
      this.midiRecording.speed = this.midiRecording.speed || [];

      this.midiRecording.extrusionMultiplier.push([currentTime, _midiController.extrusionMultiplier]);
      this.midiRecording.speed.push([currentTime, _midiController.speed]);
    }
